...
- **Default Destination**: When enabled without specifying an `exporterUrl`, telemetry data is sent to `https://api.honeycomb.io` in the `wayfinder-dev` dataset (managed by the AR.IO team) to help improve the service
...
<Tip title="Note">
  ...
  `https://api.honeycomb.io` in the `wayfinder-dev` dataset (managed by the
  AR.IO team). Specify your own `exporterUrl` to send data to your preferred
  destination.
</Tip>

### Required Configuration

The `TelemetryConfig` interface requires these fields:
...
### Root Span: `wayfinder.request`

- **Operation**: `wayfinder.request`
- **Attributes**:
  - `wayfinder.protocol`: `ar://`
  - `wayfinder.transaction_id`: Transaction ID being requested
  - `wayfinder.url`: Original ar:// URL
  - `http.status_code`: Final HTTP status code
  - `http.response.size`: Response size in bytes

### Child Span: `wayfinder.routing`

- **Operation**: `wayfinder.routing`
- **Attributes**:
  - `wayfinder.routing.strategy`: Routing strategy used
  - `wayfinder.routing.available_gateways`: Number of available gateways
  - `wayfinder.routing.selected_gateway`: Selected gateway URL
  - `wayfinder.routing.selection_time_ms`: Time taken to select gateway

### Child Span: `wayfinder.verification`

- **Operation**: `wayfinder.verification`
- **Attributes**:
  - `wayfinder.verification.enabled`: Whether verification was enabled
  - `wayfinder.verification.strategy`: Verification strategy used
  - `wayfinder.verification.result`: Verification result (success/failure)
