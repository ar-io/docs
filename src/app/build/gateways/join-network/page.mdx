...
The simplest method for joining a new gateway to the [Gateway Address Registry (GAR)](/build/gateways/gateway-network#gateway-address-registry-gar) is to use the [Network Portal](https://gateways.arweave.net/#/gateways).
...
<Diagram title="Start Your Gateway" src='47FLOGesQYP2mi5GvoAaNIZhWofDnh0rsUCWtx21W5w' />

<Diagram title="Start Gateway Form" src='IVXGB2_kWIE0IZ7MskNwD1aQZUZFN-Xa1jeZJXqg9dI' />
...
- **Observer Wallet**: This is the public wallet address of the wallet used for the gateway's [observer](/build/gateways/observer). By default, the primary gateway wallet address is filled in this space; however, a different wallet may be utilized if desired for operational reasons.
- **Properties ID**: This is an Arweave Transaction Id for a JSON object that contains additional details about the gateway. The gateway network has not yet incorporated these properties into standard gateway participation, and so the space may safely be left as the default value. The contents of the default properties Id can be viewed [here](https://arweave.net/FH1aVetOoulPGqgYukj0VE0wIhDy90WiQoV3U2PeY44)
- **Stake**: This is the amount of ARIO tokens to be staked to the gateway. It must be at least the network minimum.
- **Delegated Stake**: This toggle enables or disables [delegated staking](/learn/staking#delegated-staking) on a gateway. This may be changed later.
...
Joining the network can also be completed programmatically through the [AR.IO SDK](/build/ar-io-sdk). This is done using the [join-network](/build/ar-io-sdk/ario/gateways/join-network) method on the ARIO class.
