...
export const metadata = {
  title: 'Legacy AO Compute Unit (CU)',
  description: 'Steps for deploying a legacy AO Compute Unit (CU) sidecar alongside your AR.IO Gateway.',
  openGraph: {
    title: 'Legacy AO Compute Unit (CU)',
  },
}
...
- At least 16GB RAM dedicated to CU operation
- At least 100GB disk space dedicated to CU operation
- These requirements are separate from your gateway requirements

<Note>
Running a CU is resource-intensive. Make sure your system has sufficient resources to handle both the gateway and the CU.
</Note>

## Deploying a Legacy CU
...
1. **PROCESS_WASM_MEMORY_MAX_LIMIT**: Sets the maximum memory limit (in bytes) for WASM processes.
   ```
   PROCESS_WASM_MEMORY_MAX_LIMIT=4294967296  # Example: 4GB (4 * 1024^3)
   ```

2. **WASM_EVALUATION_MAX_WORKERS**: Sets the maximum number of worker threads for WASM evaluation. This should be limited by the number of CPU cores available.
   ```
   WASM_EVALUATION_MAX_WORKERS=4  # Example: Use 4 worker threads
   ```

3. **PROCESS_WASM_COMPUTE_MAX_LIMIT**: Sets the maximum CPU usage percentage allowed for WASM processes.
   ```
   PROCESS_WASM_COMPUTE_MAX_LIMIT=80  # Example: Limit to 80% of CPU
   ```
