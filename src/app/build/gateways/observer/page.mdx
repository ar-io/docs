...
This setup permits gateway and observer nodes to experiment and evolve best practices for performance evaluation, all while operating within the bounds of the network's immutable [smart contract](/learn/ario-contract), thus eliminating the need for frequent contract updates (forks). 
...
If the [GAR](/build/gateways/gateway-network#gateway-address-registry-gar) is below that amount, then every gateway is designated as an observer for that epoch. 
...
//TODO: Establish CSS classes for 'formula' and 'caption'
