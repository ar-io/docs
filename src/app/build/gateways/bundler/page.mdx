...
- `BUNDLER_ARWEAVE_ADDRESS` must be the [normalized public address](/learn/glossary.md#normalized-address) for the provided Arweave wallet.
...
By default, the bundler will only accept data items uploaded by data item signers whose [normalized wallet addresses](/learn/glossary.md#normalized-address) are in the `ALLOW_LISTED_ADDRESSES` list. This is an additional environmental variable that can be added to your `.env.bundler` file, and must be a comma separated list of normalized public wallet addresses for wallets that should be allowed to bundle and upload data through your gateway.
...
`$BUNDLER_ARWEAVE_ADDRESS` should be replaced with the [normalized public wallet address](/learn/glossary#normalized-address) associated with the bundler.
