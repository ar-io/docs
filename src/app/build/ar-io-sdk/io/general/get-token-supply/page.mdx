...
`getTokenSupply` is a method on the `IO` class that retrieves the total supply of tokens, returned in mIO. The total supply includes the following:
...
<CodeGroup title="getInfo">
    ...
    async function main(){
        const io = IO.init();
        const info = await io.getTokenSupply()
