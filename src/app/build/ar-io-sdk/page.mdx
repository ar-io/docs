...
The AR.IO SDK provides functionality for interacting with the AR.IO ecosystem of services and protocols. This includes, [the AR.IO Network](/learn/network-composition), [gateways](/build/gateways), [the ARIO token](/learn/token),  and [ArNS domains](/learn/arns). The AR.IO SDK is available for both NodeJS and web environments.
...
AR.IO gateways are open source nodes that index and serve Arweave transaction headers and data items. Gateway operators may join their gateway to the [Gateway Address Registry](/build/gateways/gateway-network) (GAR), which makes the gateway discoverable using the AR.IO SDK. The gateway information is stored in the AR.IO AO contract as a JSON object with the following attributes:
...
The Arweave Name System (ArNS) is a protocol which allows for assigning friendly names to Arweave transactions or data items. Powered by [Arweave Name Tokens](/learn/arns) (ANTs), AO tokens that manage settings for individual ArNS domains, ArNS enables easy interaction with data stored on Arweave.
