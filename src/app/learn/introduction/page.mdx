...
The network features modular and composable gateway infrastructure in addition to the [Arweave Name System (ArNS)](/learn/arns) – a system for assigning friendly domain names to permanent data. 
...
The [AR.IO Network](/learn/network-composition) is an open, distributed, and ownerless system, supported by operators, developers, and end-users from around the world. 
It’s decentralized nodes, known as [AR.IO Gateways](/build/gateways), act as “Permanent Cloud Service Providers” delivering the critical services needed to read, write, index and query data stored on the permaweb. 
...
They also engage in an [observation and reporting protocol](/build/gateways/observer) to monitor performance and ensure accountability, helping to maintain a healthy and reliable ecosystem. 

The AR.IO Network is powered by a [utility token](/learn/token), ARIO, which drives the network’s functionality and accessibility. 
