...
The ANTRegistry class can be initialized in two ways, depending on whether a signer is provided:
...
| Parameter  | Type           | Description                         | Optional |
|------------|----------------|-------------------------------------|----------|
| processId  | string         | ID of the AO process (defaults to ANT registry) | Yes      |
| signer     | ContractSigner | An optional signer instance, used to enable write operations on the blockchain | Yes      |
...
- Purpose: Get a list of ANT processIds for ANTs owned or controlled by a walletAddress
- Authentication: Not required
