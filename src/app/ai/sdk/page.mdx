...
#### Finding ANTs Owned or Controlled by an Address

To find ANTs associated with a wallet address, use the ANTRegistry class directly:

```typescript
// Import the ANTRegistry class
import { ANTRegistry } from '@ar.io/sdk'; // or '@ar.io/sdk/web' for browser

// Initialize the ANTRegistry (no authentication needed for read operations)
const registry = ANTRegistry.init();

// Get ANTs owned or controlled by a specific address
const result = await registry.accessControlList({
  address: "WALLET-ADDRESS-HERE"
});

// Result contains two arrays:
// result.Owned - ANT process IDs owned by this address
// result.Controlled - ANT process IDs where this address is a controller
console.log("Owned ANTs:", result.Owned);
console.log("Controlled ANTs:", result.Controlled);
```
...
- The ANTRegistry is a separate class from ARIO and ANT
- The ANTRegistry.accessControlList method is the correct way to find ANTs associated with an address
- The result contains process IDs that must be used to initialize ANT instances
- There is no method called "getANTs" on the ARIO class
- The ANTRegistry cannot be initialized with an ARIO instance
