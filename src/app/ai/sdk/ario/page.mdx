...
- Authentication: Not required
- **CRITICAL: Pagination is MANDATORY for this method. The method will only return 10 results without pagination.**
- Parameters: Pagination parameters are required

```typescript
async getGateways(
  params: PaginationParams<AoGatewayWithAddress>  // Removed optional '?' to make it required
...
// Example usage with REQUIRED pagination:
const getGatewaysExample = async () => {
  const ario = ARIO.init();
  
  // Initialize REQUIRED pagination parameters
  ...
  // Get first page of results (pagination is REQUIRED)
