...
In addition to `ar-io.dev` - you can also elect to run your own ARIO gateway that resolves names against testnet. To do so, you need to setup your by following the steps in [XXXX].
...
To prevent abuse, the ARIO testnet enforces the following restrictions and limitations.

1. Wallets are limited to 10k tARIO tokens
1. hCaptcha must be satisfied to request tokens
1. Rate limits apply to requesting tokens
1. Balances and ArNS names stored on testnet **will be pruned/reset by default, every ~30 days (subject to change)**
1. The ARIO team may prune balances and ArNS names at their discretion
