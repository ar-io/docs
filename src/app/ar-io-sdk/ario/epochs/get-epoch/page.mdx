...
`getEpoch` is a method on the `ARIO` class that returns the epoch data for the specified epoch index. If no epoch index is provided, the current epoch is used.
...
<Table>
  ...
  <Table.Body>
    <Table.Row>
      ...
      <Table.Cell>The epoch index for which to retrieve epoch data.</Table.Cell>
