...
<Tip title="New in v3.16.0: Progress Callbacks">
  The `spawnANT` utility now supports an optional `onSigningProgress` callback
  that provides real-time updates during the ANT spawning process. This callback
  receives progress steps like 'spawning' and 'verifying-ownership' along with
  relevant event data, making it easier to track the progress of ANT creation
