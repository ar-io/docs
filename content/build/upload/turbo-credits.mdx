...
| Base        | [ARIO](https://basescan.org/token/0x138746adfA52909E5920def027f5a8dc1C7EfFb6), [ETH](https://www.coingecko.com/en/coins/ethereum), [USDC](https://basescan.org/token/0x833589fcd6edb6e08f4c7c32d4f71b54bda02913) |
...
Visit [console.ar.io](https://console.ar.io) to purchase credits with fiat or crypto using a connected wallet.
...
Credits can be shared via the Console App at [console.ar.io](https://console.ar.io) or programmatically via the [Turbo CLI](https://github.com/ardriveapp/turbo-sdk/?tab=readme-ov-file#cli), [SDK](/sdks/turbo-sdk), or [API](https://payment.ardrive.io/api-docs).
...
Every ArNS operation goes through one endpoint, and the response tells you whether it is already done or needs your signature:

```js
// Price it first — quote wincTotalWithAntSpawn, not winc alone
const price = await get('/v1/arns/price/buy-name/my-name?type=lease&years=1');

// Create the action. `ownerAddress` is the Solana address that will own the ANT.
let res = await post('/v1/arns/actions/buy-name', {
  name: 'my-name',
  type: 'lease',
  years: 1,
...
// Buying is the one action that always needs the owner's signature.
...
// res.messageId is the on-chain write
```

Extending, upgrading and adding undernames need no signature at all. See the dedicated [ArNS Names (paid with Turbo Credits)](/sdks/turbo-sdk/purchase-lifecycle) guide for the full action lifecycle, pricing, record management and transfers.
