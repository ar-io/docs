...
<Steps>
  ...
  <Step>
    ...
    <Tabs items={['Node.js', 'Browser']}>
      <Tab value="Node.js">
        <Tabs items={['Arweave JWK', 'Ethereum', 'Solana', 'Polygon']}>
  ...
  <Step>
    ...
    <Tabs items={['Purchase Methods', 'Purchase with Crypto']}>
      <Tab value="Purchase Methods">
        - **Fiat Currency**: Credit/debit cards via the [Turbo Top Up App](https://console.ar.io/topup)
        - **Cryptocurrencies**: AR, ETH, SOL, MATIC, ARIO, USDC, ETH (BASE)
        - **Multiple Wallets**: Ethereum, Solana, and Arweave wallets supported

      </Tab>

      <Tab value="Purchase with Crypto">
        <Tabs items={['Arweave', 'Ethereum', 'Solana', 'Polygon']}>
          <Tab value="Arweave">
            ```typescript
  ...
  <Step>
    ...
    console.log("File uploaded!", {
      id: result.id,
      url: `https://turbo-gateway.com/${result.id}`,
      owner: result.owner,
...
<Tabs items={['Cost Management', 'Progress Tracking', 'Error Handling']}>
  <Tab value="Cost Management">
    ...
    console.log(`Upload cost: ${costs[0].winc} winc`);
    ...
    console.log(`Available credits: ${balance.controlledWinc} winc`);
    ```
