...
> **Note:** Running a CU is resource-intensive. Make sure your system has sufficient resources to handle both the gateway and the CU. While you can run a CU with less than the recommended RAM, you'll need to adjust the memory limits accordingly.
...
1. **CU_WALLET**: Replace `'[wallet json here]'` with the JSON from an Arweave wallet.
   ...
   > **Note:** The entire JSON must be placed on a single line for proper registration.
...
4. **UPLOADER_URL**: By default, this is set to use a bundler sidecar run by your gateway:
   ...
   > **Note:** Important: Checkpoint Uploads Require Payment
   >
...
1. **PROCESS_WASM_MEMORY_MAX_LIMIT**: Sets the maximum memory limit (in bytes) for WASM processes.
   ...
   > **Note:** Important Memory Requirement
   >
...
2. **WASM_EVALUATION_MAX_WORKERS**: Sets the maximum number of worker threads for WASM evaluation.
   ...
   > **Note:** Worker Thread Configuration
   >
...
> **Note:** A CU won't do anything until requests are being made of it. By connecting your gateway to the CU, you'll start generating these requests.
