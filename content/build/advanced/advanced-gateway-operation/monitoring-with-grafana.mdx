...
<Tabs items={['Local Access', 'External Access via Nginx', 'Custom Configuration']}>
...
For custom setups or specific requirements:

<Steps>
<Step>
### Modify Docker Compose

Edit `docker-compose.grafana.yaml` to customize settings:

```yaml
grafana:
  image: grafana/grafana:latest
  user: root # For permission issues
  ports:
    - "3000:3000" # Custom port mapping
  environment:
...
If using custom ports, update Nginx accordingly:
...
Set custom Grafana configuration:
...
Grafana automatically connects to the gateway's Prometheus metrics endpoint. No additional configuration is typically required.
...
To monitor multiple gateways or external services:

1. Go to "Configuration" → "Data Sources"
2. Add new Prometheus data sources
...
1. Set up separate organizations for different teams
2. Assign users to appropriate organizations
3. Configure data source access per organization
...
- **Community Support** - Join the [AR.IO Discord](https://discord.gg/7zUPfN4D6g) for help
