...
Deploy a permanent web application to Arweave with a human-readable ArNS name — using only your Solana wallet. Your app will be served by ~600 decentralized gateways with zero ongoing hosting costs.

## What You'll Build

By the end of this guide, your app will be:
- **Permanently stored** on Arweave (can never be deleted or modified)
- **Accessible** at `https://yourname.arweave.net` (and every other ar.io gateway)
...
<Steps>
  ...
  <Step>
    ...
    console.log(`  https://${ARNS_NAME}.arweave.net`);
  ...
  <Step>
    ...
    ```bash
    curl -I https://my-cool-app.arweave.net
...
// Access at: https://staging_my-cool-app.arweave.net
...
async function deploy() {
  ...
  console.log(`\nLive at: https://${ARNS_NAME}.arweave.net`);
}
