    ...
    <Steps>
      ...
      <Step>
        ...
        **Add configuration:**
        ...
        <Callout type="info">
        `AR_IO_WALLET` and `OBSERVER_WALLET` must be Solana public keys (base58 Ed25519). The observer address must be unique — no other gateway can use the same observer. **Both wallets need SOL** — the observer pays for `save_observations` instructions, and the operator pays for cranker instructions when `ENABLE_EPOCH_CRANKING=true` (`tally_weights`, `prescribe_epoch`, `distribute_epoch`, `close_epoch`). Budget ~0.5 SOL each to cover several months of activity.
        </Callout>
