    ...
    <Steps>
      ...
      <Step>
        ...
        **Parameter explanations:**
        ...
        - `-t solana --mainnet` — required to target the canonical Solana mainnet program IDs. For staging or local devnet, drop `--mainnet` and pass explicit `--core-program-id` / `--gar-program-id` / `--arns-program-id` / `--ant-program-id` + `--rpc-url`.
      </Step>
      ...
      <Step>
        ...
        ```bash
        ar.io get-gateway --mainnet -t solana --address <your-operator-pubkey>
        ```
        ...
        Mismatches here are the most common "I joined but observations aren't being submitted" cause — the gateway is configured for a different network than the one you joined.
