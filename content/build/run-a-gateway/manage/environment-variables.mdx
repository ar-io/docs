...
When `OBSERVER_KEYPAIR_PATH` or `OBSERVER_PRIVATE_KEY` is set, the observer's Ed25519 Solana key is used directly as the HTTPSIG signing key — its Solana address is registered in the on-chain Gateway Registry, so verifiers can derive trust from a single lookup (no attestation document needed). When neither is set, a standalone Ed25519 key is auto-generated at `HTTPSIG_KEY_FILE` (responses are signed but not verifiable against the registry). Setting both `OBSERVER_KEYPAIR_PATH` and `OBSERVER_PRIVATE_KEY` is rejected at startup as ambiguous.
...
The gateway talks to four separate Solana programs that together implement the ar.io protocol (see [protocol architecture](/learn/protocol-architecture)). Each program ID is configured independently so a single gateway image can run against mainnet, staging-devnet, or a local devnet without rebuilding. After startup, you can confirm which set the gateway is using via `GET /ar-io/info`, which echoes the resolved `programIds` object.
