...
Configure Coinbase Developer Platform integration:

```bash
# CDP configuration for official facilitator
...
RATE_LIMITER_ALLOWLIST_IPS=192.168.1.100,10.0.0.50

# ArNS name allowlist (comma-separated)
RATE_LIMITER_ALLOWLIST_ARNS_NAMES=trusted-app,premium-service
```
...
| Variable                                  | Description                         | Default   |
| ----------------------------------------- | ----------------------------------- | --------- |
| `RATE_LIMITER_TYPE`                       | Rate limiter type (memory or redis) | `memory`  |
...
- **Monitor Performance**: Use [Grafana](/build/extensions/grafana) to track payment metrics
- **Optimize Settings**: Adjust rate limits and pricing based on usage patterns
- **Scale Infrastructure**: Consider Redis for distributed deployments
- **Customize Experience**: Configure paywall branding and messaging

<Callout type="info">
  **Ready to monetize your gateway?** Your x402 payment system is now configured
  and ready to handle data egress payments. Monitor the metrics and adjust
