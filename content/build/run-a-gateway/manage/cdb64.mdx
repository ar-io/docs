...
<Callout type="info">
  **Default Behavior**: As of Release 67, CDB64 is enabled by default with no configuration required. As of release r70, the gateway ships three pre-built indexes by default, covering close to 3.8 billion data items combined.
</Callout>
...
3. **graphql** - GraphQL queries to trusted gateways
4. **hyperbeam** - A HyperBEAM endpoint's `/raw/{id}/offset`, resolved to a root transaction against your local database
5. **cdb** - CDB64 file-based index (O(1) lookup from local files or cached remote data)

The default configuration tries each source in order until a match is found:

```bash
ROOT_TX_LOOKUP_ORDER=db,gateways,graphql,hyperbeam,cdb
```

## Default Coverage

The gateway ships three CDB64 indexes by default:

- A non-AO index covering data items with content types (approximately 964 million data items)
- A non-AO index covering data items without content types (approximately 1.2 billion data items)
- An AO index (approximately 1.6 billion data items)

The two non-AO indexes exclude Redstone data items. All three cover block heights 0 through 1,820,000.

This means most historical ArDrive, Akord, AO, and similar application data can be resolved via the CDB64 index. The default shipped indexes store partition data on Arweave, so network requests are made to fetch CDB data (with intelligent byte-range caching). For zero network latency, you can download the CDB files locally.
...
```bash
ROOT_TX_LOOKUP_ORDER=db,gateways,graphql,hyperbeam
```

### Using Custom Index Sources

You can configure custom CDB64 index sources to supplement or replace the default indexes:
...
# Retrieval order for fetching CDB files from Arweave (default: chunks)
CDB64_REMOTE_RETRIEVAL_ORDER=chunks
```
...
./tools/verify-cdb64 --index index.cdb --gateway https://turbo-gateway.com
```
...
The default shipped indexes exclude Redstone data. For these, you'll need to:
- Generate a custom index covering the desired data
- Rely on other lookup sources (db, gateways, graphql, hyperbeam)
