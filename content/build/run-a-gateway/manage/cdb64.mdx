...
4. **hyperbeam** - A HyperBEAM offset API, when you run one
...
With local CDB64 indexes, such as bands from [Index Sharing](/build/run-a-gateway/manage/index-sharing), move `cdb` right after `db` so the local index answers before any network source: `ROOT_TX_LOOKUP_ORDER=db,cdb,gateways,graphql`.
...
## Fresher Indexes from Other Gateways

The shipped index stops at block 1,820,000. With [Index Sharing](/build/run-a-gateway/manage/index-sharing), your gateway can subscribe to another gateway's indexes instead, such as turbo-gateway.com's, which covers block 0 to the chain tip with offsets and is updated as new data arrives. Every file is signed and checked before your gateway uses it.
