...
<Steps>
  ...
  <Step>
    ...
    **Remove the old AO variables** (they are no longer used):
    ...
    <Callout type="warn">
    The default public Solana RPC (`api.mainnet-beta.solana.com`) is rate-limited and will visibly throttle your observer/cranker — most observation cycles will silently miss their submission window. Use a dedicated RPC provider in production such as [Helius](https://helius.dev), [Triton](https://triton.one), or [QuickNode](https://quicknode.com).
    </Callout>
  ...
  <Step>
    ...
    1. **Release + program IDs (cross-check config is what you intended):**
       ...
       ```
       `programIds.core/gar/arns/ant` should match the network you're targeting. A mismatch here is the most common "I migrated but nothing's working" cause.
...
```bash
ar.io get-current-epoch --mainnet -t solana
# or for staging/devnet:
ar.io get-current-epoch --rpc-url <staging-rpc> --core-program-id ... --gar-program-id ... -t solana
```
...
The cranker silently bails out if `EpochSettings.enabled` is `false`. This is by design — operators shouldn't pay SOL fees attempting cranker ix against a paused network. Confirm via `ar.io get-epoch-settings` and check whether epochs are enabled on your target network. If `enabled` is false, no cranker activity is expected.
