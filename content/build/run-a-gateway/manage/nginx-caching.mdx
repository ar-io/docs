...
If your gateway [shares indexes](/build/run-a-gateway/manage/index-sharing), give `/ar-io/indexes` its own location block, **uncached**. The gateway already marks each response correctly (errors are `no-store`, and files are `private` when you meter them), but an `/ar-io/` block with its own `proxy_cache_valid` or stale-serving rules could otherwise keep a publication a subscriber should not see. Band files are 7 to 30 MB, so turn off buffering too:

```nginx
# Longer prefix than any /ar-io/ block, so it wins.
...
<Callout type="warning">
  **More than one node?** Only the node that holds your observer key signs
  publications. Point this block's `proxy_pass` at that node on every node, so
  subscribers never reach one that doesn't publish. turbo-gateway.com runs
  this block on both of its nodes.
</Callout>
