---
title: "Manage your Gateway"
description: "Comprehensive guides for advanced ar.io Gateway operations including monitoring, optimization, content moderation, and specialized configurations"
---

import {
  Database,
  RotateCcw,
  TrendingUp,
  Zap,
  Globe,
  Shield,
  Settings,
  Wrench,
  CreditCard,
  Search,
  Bot,
  Share2,
} from "lucide-react";

Master the advanced features and configurations of your ar.io Gateway. These comprehensive guides cover everything from performance optimization to content moderation, helping you run a professional-grade gateway infrastructure.

## Gateway Management

<Cards>
<Card title="Importing SQLite Database Snapshots" href="/build/run-a-gateway/manage/index-snapshots" icon={<Database className="w-6 h-6" />}>
  Learn how to import pre-synchronized database snapshots to quickly bootstrap your gateway and reduce initial sync time from weeks to hours.
</Card>

<Card title="Upgrading a Gateway" href="/build/run-a-gateway/manage/upgrading-a-gateway" icon={<RotateCcw className="w-6 h-6" />}>
  Step-by-step guide to safely upgrade your ar.io Gateway to the latest version without losing data or progress.
</Card>
</Cards>

## Monitoring & Analytics

<Cards>
  <Card
    title="Monitoring with Grafana"
    href="/build/extensions/grafana"
    icon={<TrendingUp className="w-6 h-6" />}
  >
    Deploy and configure Grafana for comprehensive gateway monitoring,
    performance analytics, and operational insights.
  </Card>
</Cards>

## Performance Optimization

<Cards>
<Card title="Gateway Filters" href="/build/run-a-gateway/manage/filters" icon={<Zap className="w-6 h-6" />}>
  Configure advanced filters to efficiently process and index only the data you need, optimizing performance and resource usage.
</Card>

<Card title="CDB64 Root TX Index" href="/build/run-a-gateway/manage/cdb64" icon={<Search className="w-6 h-6" />}>
  Configure the CDB64 index for O(1) data item lookups. Enabled by default with ~964 million records for instant historical data resolution.
</Card>

<Card title="Index Sharing" href="/build/run-a-gateway/manage/index-sharing" icon={<Share2 className="w-6 h-6" />}>
  Subscribe to another gateway's signed indexes to find data items from block 0 to the chain tip locally, or publish your own.
</Card>

<Card
  title="Setting Apex Domain Content"
  href="/build/run-a-gateway/manage/setting-apex-domain"
  icon={<Globe className="w-6 h-6" />}
>
  Customize your gateway's root domain to serve custom content, project
  information, or documentation instead of default network info.
</Card>

<Card title="x402 Payment Setup" href="/build/run-a-gateway/manage/x402-setup" icon={<CreditCard className="w-6 h-6" />}>
  Configure x402 payment protocol to monetize data egress with frictionless cryptocurrency payments and premium access tiers.
</Card>
</Cards>

## Content Management

<Cards>
  <Card
    title="Content Moderation"
    href="/build/run-a-gateway/manage/content-moderation"
    icon={<Shield className="w-6 h-6" />}
  >
    Implement content moderation policies using blocklisting and filtering to
    control what content your gateway serves.
  </Card>
</Cards>

## Configuration Reference

<Cards>
  <Card
    title="Environment Variables Reference"
    href="/build/run-a-gateway/manage/environment-variables"
    icon={<Settings className="w-6 h-6" />}
  >
    Comprehensive reference for all ar.io Gateway environment variables
    organized by service component.
  </Card>
</Cards>

## AI-Assisted Operations

<Cards>
  <Card
    title="Claude Code Operator Skill"
    href="https://github.com/ar-io/ar-io-node/tree/develop/.claude/skills/ar-io-gateway-operator"
    icon={<Bot className="w-6 h-6" />}
  >
    The `ar-io-node` repo ships a Claude Code skill at
    `.claude/skills/ar-io-gateway-operator/` with a one-screen
    `scripts/health-check` snapshot and an operational runbook covering the
    ANS-104 pipeline, ClickHouse, ArNS resolution, observer/cranker, and
    common pitfalls. Claude Code auto-loads it when run from the repo root.
    Readable as plain Markdown if you don't use Claude Code.
  </Card>
</Cards>

## Support & Troubleshooting

<Cards>
  <Card
    title="Gateway Troubleshooting & FAQ"
    href="/build/run-a-gateway/manage/troubleshooting"
    icon={<Wrench className="w-6 h-6" />}
  >
    Comprehensive troubleshooting guide and FAQ for common gateway issues,
    failed epoch guidance, and frequently asked questions.
  </Card>
</Cards>
