...
**Primary names** are [ArNS names used as identity](https://docs.ar.io/arns#primary-names) that:
...
- Choose a unique name (e.g., `alice`)
...
- `thebestguy.arweave.dev` → `_pxu3CF4PZdSRJXgvsh-byud6Gti-UvOUA203agA0EA`
- Others can find your wallet using your name
- Use in dApps and applications

**Address to name resolution:**

- `_pxu3CF4PZdSRJXgvsh-byud6Gti-UvOUA203agA0EA` → `thebestguy`
...
- **Send tokens to "thebestguy"** instead of copying long wallet addresses
...
const nameData = await ario.getPrimaryName({
  address: "_pxu3CF4PZdSRJXgvsh-byud6Gti-UvOUA203agA0EA",
});
console.log(nameData.name); // e.g., "thebestguy"
```
...
console.log(nameData.owner); // e.g., "_pxu3CF4PZdSRJXgvsh-byud6Gti-UvOUA203agA0EA"
console.log(nameData.name); // e.g., "thebestguy"
```
...
- Send tokens to "thebestguy" instead of copying `_pxu3CF4PZdSRJXgvsh-byud6Gti-UvOUA203agA0EA`
- Apps automatically resolve the name to the wallet address
- Much more user-friendly than long wallet addresses

**User interfaces:**

- Display "thebestguy" as username when wallet is connected
