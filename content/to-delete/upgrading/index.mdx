...
If the release number displayed includes `-pre` it means that your gateway is using the `develop` branch of the github repo for the gateway code. Follow steps in our [troubleshooting guide](/gateways/troubleshooting) to switch over to the more stable `main` branch.
