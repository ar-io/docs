...
A permanent cloud network: independent gateways serve, index and verify permanent data, and ArNS gives it names. Anyone can run a gateway and join.
...
An Arweave transaction, or a data item, whose data holds a set of other data items packed in the ANS-104 format. A bundle contains data items; it is not one of them. See [Bundling](#bundling).
...
The id of a [data item](#ans-104-data-item), assigned when it is signed. A gateway can look up a data item by its data item id alone and resolve it to the root transaction that contains it.
...
An ar.io node that serves Arweave data over HTTP: it retrieves, caches and indexes data, resolves ArNS names, and exposes standard interfaces for the permaweb. Anyone can run one. Turbo is an ar.io gateway run by the ArDrive team, and it also accepts paid uploads. See [Gateways](/learn/gateways).
...
The Arweave transaction that contains a data item, found by walking up through every bundle around it. A gateway resolves a data item's root transaction to serve and locate it.
...
An ar.io gateway run by the ArDrive team. It serves permanent data like any [ar.io gateway](/learn/gateways) and adds paid uploads: it bundles your files into ANS-104 data items, issues the credits you pay with, and posts the data permanently to Arweave. See [Getting Started with Turbo](/build/upload/bundling-services).

## Turbo Credits

Prepaid upload balance, bought with a card or crypto and spent on uploads and ArNS names. Priced in [winc](#winc) and non-transferable. See [Paying for Uploads](/build/upload/turbo-credits).
...
The unit Turbo Credits are counted in, as it appears in the Turbo API. Receipt and balance fields return amounts in `winc`. Not to be confused with Winston, the smallest unit of AR.
