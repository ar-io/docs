...
One piece of a shared index, covering a range of block heights. Older bands rarely change; the band at the chain tip is rebuilt as new data arrives. A gateway installs the bands it needs and replaces one without pausing lookups. See [Index Sharing](/learn/gateways/index-sharing).

## Index Sharing

How ar.io gateways share the indexes that locate data items inside their bundles. A gateway publishes a document signed with its registered observer key, listing its index bands and every file's SHA-256; other gateways and clients verify it and use the bands without trusting the server they fetched them from. See [Index Sharing](/learn/gateways/index-sharing).
