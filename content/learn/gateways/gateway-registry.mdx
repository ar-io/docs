...
## Exploring the Gateway Registry

A full list of gateways in the AR.IO network can be explored at [gateways.ar.io](https://gateways.ar.io). The network incentivizes and observes gateway performance through ArNS (Arweave Name System) resolution testing, where gateways are evaluated based on their ability to resolve ArNS names with appropriate weights applied to different performance criteria. To learn more about how gateway performance is evaluated and the specific metrics used, see [Performance Evaluation](/learn/oip/performance-evaluation).
