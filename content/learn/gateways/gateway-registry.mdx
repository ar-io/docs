...
A registered gateway is always in one of three states: **joined**, **leaving**, or **gone**.

A gateway operator can leave voluntarily, and a gateway removed through [pruning](/learn/oip/pruning) leaves the same way:

1. **Leaving**: The gateway stops being eligible for new rewards and enters a leave window of at least 90 days, during which any delegated stake on it still has to be withdrawn.
2. **Gone**: Once the leave window has expired and all delegated stake has been claimed, a permissionless instruction closes the gateway's registry entry and frees its slot. A gone gateway has no registry entry left; its operator has to register again to rejoin.
...
- A gateway moves from joined to leaving to gone as it exits the registry, freeing its slot once it is gone.
