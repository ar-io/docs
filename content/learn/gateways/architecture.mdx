...
Gateways expose a serving layer for applications and end users. This layer includes byte-range requests, signed and verifiable responses, x402 paid access, peer routing, content moderation hooks, and [Index Sharing](/learn/gateways/index-sharing), through which gateways publish and subscribe to signed indexes that locate data items. These capabilities let operators tune how data is delivered while preserving verifiability and local operator choice.
