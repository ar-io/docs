...
## Evaluation and Distribution Timeline
...
![Epoch reward distributions showing eligible vs distributed rewards across multiple epochs](/epoch-distributions.png)

The distribution chart shows how rewards are allocated each epoch, including:
- **Eligible Rewards**: Total available rewards from the protocol balance
- **Distributed Rewards**: Actual rewards given to functional gateways and observers
- **Auto-staking**: Automatic delegation and staking mechanisms
- **Delegate Sharing**: How rewards are shared with token delegators
...
Performance evaluation is the cornerstone of the OIP system that maintains network quality. Key takeaways:

- **Majority Rule**: Gateways need ≥50% pass votes from observers to be considered functional
- **Compounding Effects**: Performance impacts GPRW and OPRW, affecting future observer selection chances
- **Long-term Consequences**: Consistent performance builds network influence through composite weight accumulation
- **Immediate Impact**: Functional gateways earn rewards; deficient ones receive nothing
- **Observer Accountability**: Observers who fail their duties face reward penalties and reduced future selection odds

Performance evaluation creates a meritocratic system where consistent quality service is rewarded with increased network participation and influence.
