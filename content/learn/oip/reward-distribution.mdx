...
```
BGR = (Epoch Reward Allocation × 0.9) / Total Gateways in the Network
```
...
```
BOR = (Epoch Reward Allocation × 0.1) / Total Selected Observers for the Epoch
```
...
```
DRi = Total Rewards × Reward Share Ratio × (Delegate's Stake / Total Delegated Stake)
```
...
- **Back to Introduction** → [OIP Introduction](/learn/oip/introduction) to review the basics
