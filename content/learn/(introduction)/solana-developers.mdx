...
import { Upload, Globe, Server, Code, FlaskConical } from 'lucide-react';
...
- **Human-readable URLs** — `yourapp.ar.io` instead of a 43-character transaction ID
...
| **ARIO tokens** | Pay for ArNS names, staking. [Get ARIO tokens here](/learn/token/get-the-token) |
...
<Callout type="info">
**Want to try before spending real ARIO?** Run this whole flow — upload, buy an ArNS name, and
resolve it — for free on the **[Testnet Sandbox](/build/testnet)**, which runs the full ar.io
stack on Solana **devnet** with a faucet-funded staging token. It's also the ideal target for AI
coding agents and CI. When you're ready for mainnet, the code below is the same minus the sandbox
endpoints.
</Callout>
...
```
https://my-cool-app.ar.io      → served by the ar.io gateway
https://my-cool-app.<any-network-gateway>  → served by that ar.io gateway
```
...
```
https://docs_my-cool-app.ar.io  → the "docs" undername
https://api_my-cool-app.ar.io   → the "api" undername
```
...
| Upload (larger files) | Pay with SOL, ARIO, or fiat. [See pricing](https://console.ar.io/pricing) |
...
<Cards>
  ...
  <Card
    ...
    description="Try upload, payments, and ArNS for free on Solana devnet before going to mainnet"
    href="/build/testnet"
