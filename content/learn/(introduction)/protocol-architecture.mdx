...
The ar.io protocol operates through four Solana programs (3+1) that work together via cross-program invocation (CPI). This architecture consolidates what was previously spread across separate processes into a coordinated set of programs with clear responsibilities.
...
### Economic Security
- **Stake Requirements**: Minimum ARIO token stake to join as a gateway operator
- **Performance-Based Pruning**: Gateways that repeatedly fail observation are removed from the network and subject to stake slashing
...
- **Transaction Fees**: Solana fees require state-changing operations to pay execution costs, helping discourage spam and unnecessary writes
...
- **Transparent State**: All onchain state is publicly verifiable via any Solana RPC or explorer
