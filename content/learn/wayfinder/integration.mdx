...
The [wayfinder-core](/sdks/wayfinder-core) library is the core protocol implementation that accepts various configuration options for setting up Wayfinder. It provides:
...
Web developers will likely be interested in [wayfinder-react](/sdks/wayfinder-react), which provides:
...
```typescript
import { WayfinderClient } from '@ar-io/wayfinder-core';
...
For detailed routing strategy options, see the [routing strategies documentation](/sdks/wayfinder/wayfinder-core/routing-strategies/#preferred-gateway--network-fallback-strategy).
...
Users of the wayfinder-extension can configure their routing and verification strategies through the extension's settings:

1. **Routing Strategies**:
   - Random gateway selection
   - Fastest gateway (based on latency)
   - Preferred gateway with fallback
   - Custom gateway list

2. **Verification Settings**:
   - Disabled (fastest, trust-based)
   - Basic (verify transaction IDs)
   - Full (complete content verification)

## Implementation Best Practices

1. **Always configure a preferred gateway** - Reduces latency and improves reliability
2. **Enable verification for sensitive content** - Ensures authenticity
3. **Monitor telemetry in production** - Understand real-world performance
4. **Handle errors gracefully** - Provide fallback UI when content can't be loaded
5. **Cache aggressively** - Reduce repeated gateway requests
