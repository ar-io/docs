...
<Callout type="info">
SOL is required for Solana transaction fees when staking, delegating, or withdrawing ARIO. Typical transaction fees are less than 0.01 SOL.
</Callout>
...
A gateway operator must stake a minimum of 10,000 ARIO to join their gateway to the network. This makes them eligible for protocol rewards and promotes network reliability. The gateway registry supports up to 3,000 gateways.

Operator rewards always auto-compound into the operator's stake — there is no toggle. To realize rewards, operators must call `decrease_operator_stake` to withdraw the excess. A gateway's total stake impacts the following epoch once excess stake is added or removed.
...
Gateways that fail observation for 30 consecutive epochs are pruned from the network. When pruned, 100% of the gateway's minimum stake is slashed and returned to the protocol balance. Remaining excess stake and delegated stakes enter the standard withdrawal process.
...
When you delegate stake to a gateway, you're increasing that gateway's total stake. This increases the gateway's chances of being selected as an observer, which means more potential rewards for both the gateway operator and you as a delegator.
...
- **Redelegation**: You can move your stake between gateways as network conditions change. Redelegation fees reset after 7 days (wall-clock time).
...
The redelegation fee follows a linear schedule (0–60%) that resets every 7 days based on wall-clock time.
...
Gateway operators and delegated stakers can shorten the standard 90-day withdrawal delay period after initiating a withdrawal; this action is subject to a dynamic fee. The fee follows a linear decay from 50% down to 10% over the 90-day period. At any point during the delay, users can choose to expedite access to their pending withdrawal tokens by paying the fee to the protocol balance. Once triggered, the tokens are returned immediately to the user's wallet.
