...
Factory function that creates a read-only or writeable client. By providing a `signer`, additional write APIs that require signing (like `joinNetwork` and `delegateStake`) become available. By default, a read-only client is returned.

For write operations, create a signer using `@solana/kit` (install with `npm install @solana/kit`).

```typescript
import { ARIO } from '@ar.io/sdk';

// read-only client (Solana mainnet)
...
// read-write client for node environments (Solana keypair JSON file)
...
```typescript
// read-write client for browser environments (Phantom, Solflare, etc.)
const ario = ARIO.init({ signer: walletAdapter });
```

> **Legacy AO backend:** For backwards compatibility, Arweave signers (`ArweaveSigner`, `ArConnectSigner`) can be used with the `--ao` CLI flag or AO process configuration.

#### getInfo()

Retrieves the information of the ARIO protocol.
...
Retrieves the balances of the ARIO protocol in `mARIO`, paginated and sorted by the specified criteria. The `cursor` used for pagination is the last wallet address from the previous request.
