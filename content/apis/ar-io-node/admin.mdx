...
_openapi:
  toc:
    ...
    - depth: 2
      title: Optimistically index a signed L1 transaction before it mines
      url: '#optimistically-index-a-signed-l1-transaction-before-it-mines'
    - depth: 2
    ...
    - depth: 2
      title: Get latest Parquet export status (singleton)
      url: '#get-latest-parquet-export-status-singleton'
    - depth: 2
      title: Get Parquet export status for a specific job
      url: '#get-parquet-export-status-for-a-specific-job'
    - depth: 2
  ...
  structuredData:
    headings:
      ...
      - content: Optimistically index a signed L1 transaction before it mines
        id: optimistically-index-a-signed-l1-transaction-before-it-mines
      ...
      - content: Get latest Parquet export status (singleton)
        id: get-latest-parquet-export-status-singleton
      - content: Get Parquet export status for a specific job
        id: get-parquet-export-status-for-a-specific-job
    ...
    contents:
      ...
      - content: >
          ...
          resolvable (GraphQL `transaction(id)`, returning `block: null`) before
          it

          is mined. Trusted/allowlist only: requires the admin bearer token AND

          `OPTIMISTIC_TX_INDEXING_ENABLED=true` (disabled by default → 403).
          Every

          transaction is cryptographically verified
          (`arweave.transactions.verify`),

          so a forged id or `data_root` is rejected. The row is inserted with a
          NULL

          height and promoted in place when the transaction mines; never-mined
          rows

          are reclaimed by the stale-transaction GC. The data is never marked

          `verified` while the transaction is unmined (`height IS NULL`).
          Accepts a single

          transaction object or an array, up to `OPTIMISTIC_TX_MAX_BATCH_SIZE`.
        heading: optimistically-index-a-signed-l1-transaction-before-it-mines
      - content: >
      ...
      - content: |
        ...
        heading: get-latest-parquet-export-status-singleton
      - content: |
          ...
          return 404.
        heading: get-parquet-export-status-for-a-specific-job
      - content: >
